...
print('Reponse token:', login_response_data)

get_user_headers = {
    "Authorization": f"Bearer {login_response_data["token"]["accessToken"]}"
}

delete_user_response = httpx.delete(f"http://127.0.0.1:8000/api/v1/users/{create_user_response_data_user_id}", headers=get_user_headers)
delete_user_response_data = delete_user_response.json()

print('Delete user response:', delete_user_response_data)
